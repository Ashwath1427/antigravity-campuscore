-- ============================================================
-- CAMPUSCORE - SUPABASE DATABASE SETUP
...
CREATE TABLE IF NOT EXISTS cc_messages (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    sender_id UUID REFERENCES cc_users(id),
    recipient_id UUID REFERENCES cc_users(id),
...
CREATE TABLE IF NOT EXISTS cc_issues (
    ...
    student_id UUID REFERENCES cc_students(id),
    reporter_id UUID REFERENCES cc_users(id),
...
-- System Logs table
CREATE TABLE IF NOT EXISTS cc_system_logs (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_cc_messages_recipient ON cc_messages(recipient_id);
CREATE INDEX IF NOT EXISTS idx_cc_messages_status ON cc_messages(status);

-- Issues indexes
CREATE INDEX IF NOT EXISTS idx_cc_issues_student ON cc_issues(student_id);
...
ALTER TABLE cc_schedule ENABLE ROW LEVEL SECURITY;
ALTER TABLE cc_attendance ENABLE ROW LEVEL SECURITY;
ALTER TABLE cc_homework ENABLE ROW LEVEL SECURITY;
ALTER TABLE cc_exams ENABLE ROW LEVEL SECURITY;
ALTER TABLE cc_marks ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE cc_issue_timeline ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE cc_system_logs ENABLE ROW LEVEL SECURITY;

-- Basic RLS policies (you can customize these based on your security requirements)

-- Users can read their own profile
CREATE POLICY "Users can view own profile" ON cc_users
    FOR SELECT USING (auth.uid()::text = username::text);

-- Users can update their own profile
CREATE POLICY "Users can update own profile" ON cc_users
    FOR UPDATE USING (auth.uid()::text = username::text);

-- Teachers can view students in their classes
CREATE POLICY "Teachers can view their students" ON cc_students
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM cc_teachers 
            WHERE cc_teachers.name = current_setting('app.current_user_name', '')
            AND cc_students.class = ANY(string_to_array(cc_teachers.classes, ','))
        )
    );

-- All authenticated users can read announcements
CREATE POLICY "Authenticated users can read announcements" ON cc_announcements
...
CREATE TRIGGER update_cc_users_updated_at BEFORE UPDATE ON cc_users
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_students_updated_at BEFORE UPDATE ON cc_students
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_teachers_updated_at BEFORE UPDATE ON cc_teachers
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_schedule_updated_at BEFORE UPDATE ON cc_schedule
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_homework_updated_at BEFORE UPDATE ON cc_homework
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_exams_updated_at BEFORE UPDATE ON cc_exams
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_announcements_updated_at BEFORE UPDATE ON cc_announcements
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_events_updated_at BEFORE UPDATE ON cc_events
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_issues_updated_at BEFORE UPDATE ON cc_issues
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_fees_updated_at BEFORE UPDATE ON cc_fees
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_documents_updated_at BEFORE UPDATE ON cc_documents
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_cc_settings_updated_at BEFORE UPDATE ON cc_settings
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE OR REPLACE VIEW campuscore_summary AS
SELECT 
