...
function vpApplyAttendanceFilter() {
  const section = document.getElementById('section-vp_attendance');
  if (!section) return;
  const selects = section.querySelectorAll('select');
  const classVal = selects[0] ? selects[0].value : '';
  const sectionVal = selects[1] ? selects[1].value : '';
  const tbody = section.querySelector('tbody');
  if (!tbody) return;

  const rows = tbody.querySelectorAll('tr');
  rows.forEach(row => {
    const classCell = row.querySelector('.user-row-info span');
    if (!classCell) return;
    const studentClass = classCell.textContent.trim();

    let show = true;
    if (classVal.includes('Class 10')) {
      show = studentClass.startsWith('10');
    } else if (classVal.includes('Class 9')) {
      show = studentClass.startsWith('9');
    }
    if (show && sectionVal && !sectionVal.includes('All')) {
      const sec = sectionVal.replace('Section ', '');
      show = studentClass.includes('-' + sec);
    }
    row.style.display = show ? '' : 'none';
  });
  simulateAction('Filter applied');
}

// ========== FIX 6: PROFILE EDIT & CHANGE PASSWORD ==========
function openProfileEditModal() {
  const name = currentUser.name || '';
  const email = currentUser.email || '';
  const phone = currentUser.phone || '';
  const m = `<div class="modal-overlay" id="prof-edit-modal" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:420px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px">
        <h3 style="margin:0">Edit Profile</h3>
        <button onclick="document.getElementById('prof-edit-modal').remove()" style="background:none;border:none;font-size:18px;cursor:pointer;color:var(--color-text)">&times;</button>
      </div>
      <div class="form-group">
        <label>Full Name</label>
        <input type="text" id="pe-name" class="form-control" value="${name}">
        <label style="margin-top:10px">Email</label>
        <input type="email" id="pe-email" class="form-control" value="${email}">
        <label style="margin-top:10px">Phone</label>
        <input type="tel" id="pe-phone" class="form-control" value="${phone}">
        <div style="display:flex;gap:10px;margin-top:15px">
          <button class="btn-primary" style="flex:1" onclick="
            currentUser.name = document.getElementById('pe-name').value;
            currentUser.email = document.getElementById('pe-email').value;
            currentUser.phone = document.getElementById('pe-phone').value;
            localStorage.setItem('campuscore_profile_' + currentUser.username, JSON.stringify({name: currentUser.name, email: currentUser.email, phone: currentUser.phone}));
            document.getElementById('prof-edit-modal').remove();
            simulateAction('Profile updated successfully');
            triggerLiveReRender();
          ">Save Changes</button>
          <button style="flex:1;background:var(--color-surface-2);border:1px solid var(--color-border);color:var(--color-text);border-radius:8px;cursor:pointer" onclick="document.getElementById('prof-edit-modal').remove()">Cancel</button>
        </div>
      </div>
    </div>
  </div>`;
...
function openChangePasswordModal() {
  const m = `<div class="modal-overlay" id="chpw-modal" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:400px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px">
        <h3 style="margin:0">Change Password</h3>
        <button onclick="document.getElementById('chpw-modal').remove()" style="background:none;border:none;font-size:18px;cursor:pointer;color:var(--color-text)">&times;</button>
      </div>
      <div class="form-group">
        <label>Current Password</label>
        <input type="password" id="pw-cur" class="form-control">
        <label style="margin-top:10px">New Password</label>
        <input type="password" id="pw-new" class="form-control" oninput="vpValidatePw()">
        <label style="margin-top:10px">Confirm New Password</label>
        <input type="password" id="pw-conf" class="form-control" oninput="vpValidatePw()">
        <div id="pw-err" style="color:var(--color-danger);font-size:12px;margin-top:6px;display:none">Passwords do not match</div>
        <div style="display:flex;gap:10px;margin-top:15px">
          <button class="btn-primary" style="flex:1" onclick="vpSubmitPassword()">Update Password</button>
          <button style="flex:1;background:var(--color-surface-2);border:1px solid var(--color-border);color:var(--color-text);border-radius:8px;cursor:pointer" onclick="document.getElementById('chpw-modal').remove()">Cancel</button>
        </div>
      </div>
    </div>
  </div>`;
...
function vpValidatePw() {
  const n = document.getElementById('pw-new').value;
  const c = document.getElementById('pw-conf').value;
  const err = document.getElementById('pw-err');
  if (c && n !== c) { err.style.display = 'block'; err.textContent = 'Passwords do not match'; }
  else if (n && n.length < 6) { err.style.display = 'block'; err.textContent = 'Minimum 6 characters'; }
  else { err.style.display = 'none'; }
}

function vpSubmitPassword() {
  const cur = document.getElementById('pw-cur').value;
  const n = document.getElementById('pw-new').value;
  const c = document.getElementById('pw-conf').value;
  if (!cur || !n || !c) { simulateAction('Please fill all fields'); return; }
  if (n !== c) { simulateAction('Passwords do not match'); return; }
  if (n.length < 6) { simulateAction('Password must be at least 6 characters'); return; }
  document.getElementById('chpw-modal').remove();
  simulateAction('Password updated successfully');
}

// ========== FIX 7: SETTINGS BUTTONS ==========
function openLanguageModal() {
  const m = `<div class="modal-overlay" id="lang-modal" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:300px">
      ...
      <div class="form-group">
        <select id="lang-sel" class="form-control">
        ...
        <div style="display:flex;gap:10px;margin-top:15px">
          <button class="btn-primary" style="flex:1" onclick="
            localStorage.setItem('campuscore_language', document.getElementById('lang-sel').value);
...
function downloadMyData() {
  const data = {
    profile: { name: currentUser.name, email: currentUser.email, phone: currentUser.phone, role: currentUser.role },
    settings: JSON.parse(localStorage.getItem('campuscore_settings') || '{}'),
    exportDate: new Date().toISOString()
  };
  const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url; a.download = 'campuscore_vp_data.json';
  document.body.appendChild(a); a.click(); a.remove();
  URL.revokeObjectURL(url);
  simulateAction('Your data has been downloaded');
}

// ========== FIX 8: EVENTS MASTER PLAN ==========
function openEventMasterPlan(index) {
  const ev = EVENTS[index];
  if (!ev) return;
  const isApproved = ev.title.includes('Sports') ? false : true;
  const readiness = isApproved ? 100 : 80;
  const status = isApproved ? 'Approved' : 'Stage 2/4';
  const category = ev.title.includes('Sports') ? 'Sports' : ev.title.includes('Science') ? 'Academic' : ev.title.includes('Cultural') ? 'Cultural' : 'General';

  const stages = [
    { name: 'Initial Planning', done: true },
    { name: 'Resource Allocation', done: true },
    { name: 'Rehearsal / Dry Run', done: isApproved },
    { name: 'Final Execution', done: isApproved }
  ];
  const stageHTML = stages.map((s, i) => `
    <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--color-border)">
      <span style="font-size:16px">${s.done ? 'âœ…' : 'â³'}</span>
      <span style="flex:1;font-size:13px;color:var(--color-text)">Stage ${i + 1}: ${s.name}</span>
      <span class="badge ${s.done ? 'badge-active' : 'badge-pending'}" style="font-size:10px">${s.done ? 'Complete' : 'Pending'}</span>
    </div>`).join('');

  const m = `<div class="modal-overlay" id="event-plan-modal" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:550px;width:100%">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px">
        <h3 style="margin:0">${ev.title} â€” Master Plan</h3>
        <button onclick="document.getElementById('event-plan-modal').remove()" style="background:none;border:none;font-size:18px;cursor:pointer;color:var(--color-text)">&times;</button>
      </div>
      <div style="display:flex;gap:8px;margin-bottom:12px;flex-wrap:wrap">
        <span class="badge" style="background:${ev.color};font-size:10px;padding:4px 10px">${category}</span>
        <span class="badge ${isApproved ? 'badge-active' : 'badge-warning'}" style="font-size:10px">${status}</span>
      </div>
      <div style="font-size:13px;color:var(--color-text-muted);margin-bottom:12px">
        <div>ðŸ“… Date: ${ev.date}</div>
        <div>ðŸ‘¤ In-charge: A. Sharma</div>
        <div>ðŸ‘©â€ðŸŽ“ Target Classes: 6A - 10L</div>
      </div>
      <div style="margin-bottom:16px">
        <div style="display:flex;justify-content:space-between;margin-bottom:6px"><span style="font-size:13px;font-weight:600">Readiness</span><span style="font-weight:700;color:var(--color-primary)">${readiness}%</span></div>
        <div class="progress-bar"><div class="progress-fill" style="width:${readiness}%"></div></div>
      </div>
      <h4 style="margin-bottom:8px;font-size:14px;color:var(--color-text)">Preparation Stages</h4>
      ${stageHTML}
      <div style="margin-top:16px">
        <h4 style="font-size:14px;margin-bottom:6px;color:var(--color-text)">Notes</h4>
        <textarea class="form-control" rows="2" placeholder="Add notes...">${ev.desc || ''}</textarea>
      </div>
      <div style="display:flex;gap:10px;margin-top:16px">
        ${!isApproved ? '<button class="btn-primary" style="flex:1" onclick="document.getElementById(\'event-plan-modal\').remove(); simulateAction(\'Event approved successfully\')">Approve Event</button>' : ''}
        <button style="flex:1;background:var(--color-surface-2);border:1px solid var(--color-border);color:var(--color-text);border-radius:8px;cursor:pointer" onclick="document.getElementById('event-plan-modal').remove()">Close</button>
      </div>
    </div>
  </div>`;
  document.body.insertAdjacentHTML('beforeend', m);
...
// ========== FIX 9: CLASS PERFORMANCE DETAIL ==========
function openClassPerfDetail(className) {
  const perf = (typeof CLASS_PERFORMANCE !== 'undefined') ? CLASS_PERFORMANCE.find(c => c.class === className) : null;
  const data = perf || { class: className, teacher: 'N/A', avgAtt: 90, avgGPA: 7.5, weak: 2, issues: 1, topper: 'N/A' };

  const subjects = [
    { name: 'Mathematics', avg: 78, grade: 'B+' },
    { name: 'Science', avg: 82, grade: 'A' },
    { name: 'English', avg: 75, grade: 'B+' },
    { name: 'Social Studies', avg: 70, grade: 'B' },
    { name: 'Hindi', avg: 85, grade: 'A' }
  ];
  const subRows = subjects.map(s => `<tr><td style="padding:8px;border-bottom:1px solid var(--color-border)">${s.name}</td><td style="padding:8px;border-bottom:1px solid var(--color-border)"><div class="progress-bar" style="width:120px;display:inline-block;vertical-align:middle"><div class="progress-fill" style="width:${s.avg}%"></div></div> <strong>${s.avg}%</strong></td><td style="padding:8px;border-bottom:1px solid var(--color-border)"><span class="badge badge-active">${s.grade}</span></td></tr>`).join('');

  const m = `<div class="modal-overlay" id="cp-detail-modal" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:550px;width:100%">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px">
        <h3 style="margin:0">${data.class} â€” Performance Detail</h3>
        <button onclick="document.getElementById('cp-detail-modal').remove()" style="background:none;border:none;font-size:18px;cursor:pointer;color:var(--color-text)">&times;</button>
      </div>
      <div style="display:flex;gap:20px;margin-bottom:16px;flex-wrap:wrap">
        <div style="text-align:center"><div style="font-size:28px;font-weight:900;color:var(--color-primary)">${data.avgGPA}</div><div style="font-size:11px;color:var(--color-text-muted)">Avg GPA</div></div>
        <div style="text-align:center"><div style="font-size:28px;font-weight:900;color:#5ca870">${data.avgAtt}%</div><div style="font-size:11px;color:var(--color-text-muted)">Avg Attendance</div></div>
        <div style="text-align:center"><div style="font-size:28px;font-weight:900;color:#f57c00">${data.weak}</div><div style="font-size:11px;color:var(--color-text-muted)">Weak Students</div></div>
      </div>
      <h4 style="font-size:14px;margin-bottom:8px">Subject-wise Breakdown</h4>
      <table style="width:100%;border-collapse:collapse;margin-bottom:16px"><thead><tr><th style="padding:8px;border-bottom:2px solid var(--color-border);text-align:left">Subject</th><th style="padding:8px;border-bottom:2px solid var(--color-border);text-align:left">Average</th><th style="padding:8px;border-bottom:2px solid var(--color-border);text-align:left">Grade</th></tr></thead><tbody>${subRows}</tbody></table>
      <h4 style="font-size:14px;margin-bottom:6px">ðŸ† Top Performer: ${data.topper}</h4>
      <div style="display:flex;gap:10px;margin-top:16px">
        <button class="btn-primary" style="flex:1" onclick="simulateAction('Report generated')">Generate Report</button>
        <button style="flex:1;background:var(--color-surface-2);border:1px solid var(--color-border);color:var(--color-text);border-radius:8px;cursor:pointer" onclick="document.getElementById('cp-detail-modal').remove()">Close</button>
      </div>
    </div>
  </div>`;
  document.body.insertAdjacentHTML('beforeend', m);
...
function openTeacherProfileModal(teacherName) {
  const teacher = (typeof TEACHERS !== 'undefined') ? TEACHERS.find(t => t.name === teacherName) : null;
  const data = teacher || { name: teacherName, subject: 'N/A', classes: 'N/A', phone: 'N/A', status: 'Active', id: 'T000' };

  const m = `<div class="modal-overlay" id="teacher-prof-modal" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:450px;width:100%">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px">
        <h3 style="margin:0">${data.name} â€” Profile</h3>
        <button onclick="document.getElementById('teacher-prof-modal').remove()" style="background:none;border:none;font-size:18px;cursor:pointer;color:var(--color-text)">&times;</button>
      </div>
      <div style="display:flex;align-items:center;gap:15px;margin-bottom:16px">
        <div class="avatar" style="width:60px;height:60px;font-size:22px;background:var(--color-primary)">${getInitials(data.name)}</div>
        <div>
          <h4 style="margin:0;color:var(--color-text)">${data.name}</h4>
          <div style="font-size:13px;color:var(--color-text-muted)">${data.id} Â· ${data.subject}</div>
          <span class="badge ${data.status === 'Active' ? 'badge-active' : 'badge-warning'}" style="margin-top:4px">${data.status}</span>
        </div>
      </div>
      <div style="border-top:1px solid var(--color-border);padding-top:12px">
        <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--color-border);font-size:13px"><span style="color:var(--color-text-muted)">Subject</span><strong>${data.subject}</strong></div>
        <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--color-border);font-size:13px"><span style="color:var(--color-text-muted)">Classes</span><strong>${data.classes}</strong></div>
        <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--color-border);font-size:13px"><span style="color:var(--color-text-muted)">Phone</span><strong>${data.phone}</strong></div>
        <div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--color-border);font-size:13px"><span style="color:var(--color-text-muted)">Attendance</span><strong style="color:#5ca870">95%</strong></div>
        <div style="display:flex;justify-content:space-between;padding:8px 0;font-size:13px"><span style="color:var(--color-text-muted)">Performance</span><strong style="color:var(--color-primary)">Good</strong></div>
      </div>
      <div style="display:flex;gap:10px;margin-top:16px">
        <button class="btn-primary" style="flex:1" onclick="document.getElementById('teacher-prof-modal').remove(); openTeacherMsgModal('${data.name}')"><i class="fas fa-envelope"></i> Send Message</button>
        <button style="flex:1;background:var(--color-surface-2);border:1px solid var(--color-border);color:var(--color-text);border-radius:8px;cursor:pointer" onclick="document.getElementById('teacher-prof-modal').remove()">Close</button>
      </div>
    </div>
  </div>`;
  document.body.insertAdjacentHTML('beforeend', m);
...
function openTeacherMsgModal(teacherName) {
  const m = `<div class="modal-overlay" id="t-msg-modal" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:400px">
      <h3 style="margin-top:0">Message to ${teacherName}</h3>
      <div class="form-group">
        <label>Subject</label><input type="text" class="form-control" placeholder="Enter subject">
        <label style="margin-top:10px">Message</label><textarea class="form-control" rows="4" placeholder="Type your message..."></textarea>
        <div style="display:flex;gap:10px;margin-top:15px">
          <button class="btn-primary" style="flex:1" onclick="document.getElementById('t-msg-modal').remove(); simulateAction('Message sent to ${teacherName}')">Send</button>
          <button style="flex:1;background:var(--color-surface-2);border:1px solid var(--color-border);color:var(--color-text);border-radius:8px;cursor:pointer" onclick="document.getElementById('t-msg-modal').remove()">Cancel</button>
        </div>
      </div>
    </div>
  </div>`;
...
// ========== FIX 11: EXAMS - Schedule Exam & View Exam Plan ==========
function openScheduleExamModal() {
  const m = `<div class="modal-overlay" id="sched-exam-modal" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:450px;width:100%">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px">
        <h3 style="margin:0">Schedule Exam</h3>
        <button onclick="document.getElementById('sched-exam-modal').remove()" style="background:none;border:none;font-size:18px;cursor:pointer;color:var(--color-text)">&times;</button>
      </div>
      <div class="form-group">
        <label>Exam Type</label>
        <select class="form-control"><option>Mid-Term</option><option>Finals</option><option>Unit Test</option></select>
        <label style="margin-top:10px">Subject</label>
        <select class="form-control"><option>All Subjects</option><option>Mathematics</option><option>Science</option><option>English</option></select>
        <label style="margin-top:10px">Class</label>
        <select class="form-control"><option>All Classes</option><option>Class 10</option><option>Class 9</option><option>Class 8</option></select>
        <label style="margin-top:10px">Start Date</label>
        <input type="date" class="form-control">
        <label style="margin-top:10px">End Date</label>
        <input type="date" class="form-control">
        <div style="display:flex;gap:10px;margin-top:15px">
          <button class="btn-primary" style="flex:1" onclick="document.getElementById('sched-exam-modal').remove(); simulateAction('Exam scheduled successfully')">Schedule</button>
          <button style="flex:1;background:var(--color-surface-2);border:1px solid var(--color-border);color:var(--color-text);border-radius:8px;cursor:pointer" onclick="document.getElementById('sched-exam-modal').remove()">Cancel</button>
        </div>
      </div>
    </div>
  </div>`;
...
function openExamPlanModal() {
  const m = `<div class="modal-overlay" id="exam-plan-modal" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:550px;width:100%">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px">
        <h3 style="margin:0">Full Exam Plan â€” Mid-Term 2026</h3>
        <button onclick="document.getElementById('exam-plan-modal').remove()" style="background:none;border:none;font-size:18px;cursor:pointer;color:var(--color-text)">&times;</button>
      </div>
      <table style="width:100%;border-collapse:collapse;margin-bottom:16px">
        <thead><tr><th style="padding:8px;border-bottom:2px solid var(--color-border);text-align:left">Date</th><th style="padding:8px;border-bottom:2px solid var(--color-border);text-align:left">Subject</th><th style="padding:8px;border-bottom:2px solid var(--color-border);text-align:left">Class</th><th style="padding:8px;border-bottom:2px solid var(--color-border);text-align:left">Status</th></tr></thead>
        <tbody>
          <tr><td style="padding:8px;border-bottom:1px solid var(--color-border)">Apr 10</td><td style="padding:8px;border-bottom:1px solid var(--color-border)">Mathematics</td><td style="padding:8px;border-bottom:1px solid var(--color-border)">All</td><td style="padding:8px;border-bottom:1px solid var(--color-border)"><span class="badge badge-active">Ready</span></td></tr>
          <tr><td style="padding:8px;border-bottom:1px solid var(--color-border)">Apr 12</td><td style="padding:8px;border-bottom:1px solid var(--color-border)">Science</td><td style="padding:8px;border-bottom:1px solid var(--color-border)">All</td><td style="padding:8px;border-bottom:1px solid var(--color-border)"><span class="badge badge-active">Ready</span></td></tr>
          <tr><td style="padding:8px;border-bottom:1px solid var(--color-border)">Apr 14</td><td style="padding:8px;border-bottom:1px solid var(--color-border)">English</td><td style="padding:8px;border-bottom:1px solid var(--color-border)">All</td><td style="padding:8px;border-bottom:1px solid var(--color-border)"><span class="badge badge-pending">Pending</span></td></tr>
          <tr><td style="padding:8px;border-bottom:1px solid var(--color-border)">Apr 16</td><td style="padding:8px;border-bottom:1px solid var(--color-border)">Social Studies</td><td style="padding:8px;border-bottom:1px solid var(--color-border)">All</td><td style="padding:8px;border-bottom:1px solid var(--color-border)"><span class="badge badge-pending">Pending</span></td></tr>
        </tbody>
      </table>
      <button style="width:100%;background:var(--color-surface-2);border:1px solid var(--color-border);color:var(--color-text);border-radius:8px;cursor:pointer;padding:10px" onclick="document.getElementById('exam-plan-modal').remove()">Close</button>
    </div>
  </div>`;
  document.body.insertAdjacentHTML('beforeend', m);
}

// ========== FIX 12: REPORTS - Generate Report Modal ==========
function openGenerateReportModal() {
  const m = `<div class="modal-overlay" id="gen-report-modal" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:450px;width:100%">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:15px">
        <h3 style="margin:0">Generate Report</h3>
        <button onclick="document.getElementById('gen-report-modal').remove()" style="background:none;border:none;font-size:18px;cursor:pointer;color:var(--color-text)">&times;</button>
      </div>
      <div class="form-group">
        <label>Report Type</label>
        <select class="form-control"><option>Attendance</option><option>Academic</option><option>Discipline</option><option>Overall</option></select>
        <label style="margin-top:10px">Class</label>
        <select class="form-control"><option>All Classes</option><option>Class 10</option><option>Class 9</option><option>Class 8</option><option>Class 7</option><option>Class 6</option></select>
        ...
        <select class="form-control"><option>PDF</option><option>Excel</option></select>
        <div style="display:flex;gap:10px;margin-top:15px">
          <button class="btn-primary" style="flex:1" onclick="document.getElementById('gen-report-modal').remove(); simulateAction('Report generated successfully')"><i class="fas fa-file-pdf"></i> Generate</button>
          <button style="flex:1;background:var(--color-surface-2);border:1px solid var(--color-border);color:var(--color-text);border-radius:8px;cursor:pointer" onclick="document.getElementById('gen-report-modal').remove()">Cancel</button>
        </div>
      </div>
    </div>
  </div>`;
...
function openVPCaseDrawer(studentName, issue, card, idx) {
  ...
  const m = `<div class="modal-overlay" id="case-drawer" style="display:flex" onclick="if(event.target===this) this.remove()">
    <div class="modal" style="max-width:580px;width:100%;max-height:90vh;overflow-y:auto">
      ...
      <h4 style="font-size:14px;margin-bottom:8px">ðŸ“‹ Timeline</h4>
